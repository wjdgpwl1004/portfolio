...
$(document).ready(function() {
	...
	if($(".sec_04 .js_slide").size() != 0){
		js_banner_size($(".sec_04 .js_slide"));
	}
	$(window).resize(function(){
		js_banner_size($(".sec_04 .js_slide"));
	});
...
function js_banner_size(obj){
	...
	}else if(win_wid <= 1024 && win_wid > 740){
		obj.move.ul.li.width((parseInt(obj.move.width()/3)-1)+"px");	
	}else if(win_wid <= 740 && win_wid > 540){
		obj.move.ul.li.width((parseInt(obj.move.width()/2)-1)+"px");	
	}else if(win_wid <= 540 ){
		obj.move.ul.li.width((parseInt(obj.move.width()/1)-1)+"px");	
	}
...
var scroll = {
		...
		contents: function () {
			...
			if (width <= 740) {
				...
				if (!scroll.swipers.contents) {
					scroll.swipers.contents = scroll.createSwiper('.contents_list>.list_group', 1, true, false);
				}
			} else {
				if (width > 740) {
		...
		createSwiper: function (targetClass, slidesPerView, useScrollBar, isNews) {
			var options = {
