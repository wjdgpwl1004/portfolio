...
function tab_slides(){
	var tab_ul = $(".tab_slider .tab_cont .slider_cont");

	tab_ul.each(function(index){
		tabRealSlide($(this), index);
	});

}

function tabRealSlide(tab, index){
	var tab_cont = tab;

	if (!window.tab_slick) { window.tab_slick = []; }

	if (!window.tab_slick[index]) {
		window.tab_slick[index] = tab_cont.slick({
			centerMode: true,
			slidesToShow: 3,
			dots: false,
			autoplay: false,
...
$(document).ready(function() {
	scroll.wrappers.contents = $('.contents_list');
	scroll.init();
...
var scroll = {
		...
		contents: function () {
			...
			var $container= $contentsWrapper.find('>.list_group');
			var $wrapper = $container.find('>ul');
			var $slides = $wrapper.find('>li');
	
			if (width <= 740) {
				scroll.addClass($container, $wrapper, $slides);
				if (!scroll.swipers.contents) {
					scroll.swipers.contents = scroll.createSwiper('.contents_list>.list_group', 1);
				}
			} else {
				if (width > 740) {
					scroll.removeClass($container, $wrapper, $slides);
					if (scroll.swipers.contents) {
		...
		createSwiper: function (targetClass, slidesPerView) {
			var options = {
			...
			return new Swiper(targetClass, options);
		}
	};
